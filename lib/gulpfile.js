...
var pathSrcs = {
      ...
      'css':path.join(screwutil.workSpace,assetPath,'css','**','*.css'),
...
function checkFile(ftype){
  ...
  return through.obj(function (file, env, cb) {
    ...
    if (file.isStream()) {
      cb(new gutil.PluginError('screw', 'Streaming not supported'));
      return;
    }
...
function screwrev(htmlpath,size){
  ...
  return through.obj(function (file, env, cb) {
    if (!file && typeof file !=='object') {
      return cb(new gutil.PluginError('screw','丢失文件参数或者文件参数不是对象'));
    }
    if (file.isStream()) {
      return cb(new gutil.PluginError('screw','暂不支持stream'));
    }
    if(!file.contents){
      return cb();
    }
