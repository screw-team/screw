...
var pathSrcs = {
      ...
      'cssPath': path.join(screwutil.workSpace,assetPath,'css'),
      ...
      'sprite': path.join(screwutil.workSpace,assetPath,'image','sprite','*.png'),
...
gulp.task('clean', function(){
  return del([
    ...
    pathDsts.css,
    pathDsts.image,
    path.join(pathDsts.html, "*.html")
  ]);
});

//@TODO js 替换为 webpack 打包
...
gulp.task('sprite', function () {
  ...
  .pipe($.if('*.png', gulp.dest(pathDsts.image), gulp.dest(pathSrcs.cssPath)));
});
...
gulp.task('default', function(){
  runSequence.apply(this, config.watch);
});
/******************************以下是自定义gulp任务*/
/**
